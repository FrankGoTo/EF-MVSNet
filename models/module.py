...
class BasicBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1 = ConvBnReLU(in_channels, 4, kernel_size=3, stride=1, pad=1)
        self.conv2 = ConvBn(4, out_channels, kernel_size=3, stride=1, pad=1)
...
def get_edge_index(depth, depth_values):
    b,d = depth_values.shape
    _min, _max = depth_values[0,0], depth_values[0,-1]
    # print(depht)
    index = (depth - _min)*d / (_max - _min)
    index[index >= d] = d-1
    index[index < 0] = 0
    return index
