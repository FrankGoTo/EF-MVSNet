...
class MVSDataset(Dataset):
    ...
    def read_mask_hr(self, filename):
        ...
        h, w = np_img.shape
        np_img_ms = {
            "1x": cv2.resize(np_img, (w//4, h//4), interpolation=cv2.INTER_NEAREST),
            "2x": cv2.resize(np_img, (w//2, h//2), interpolation=cv2.INTER_NEAREST),
            "4x": np_img,
        }
        return np_img_ms
    ...
    def read_depth_hr(self, filename):
        ...
        h, w = depth_lr.shape
        depth_lr_ms = {
            "1x": cv2.resize(depth_lr, (w//4, h//4), interpolation=cv2.INTER_NEAREST),
            "2x": cv2.resize(depth_lr, (w//2, h//2), interpolation=cv2.INTER_NEAREST),
            "4x": depth_lr,
        }
        return depth_lr_ms
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                depth_values = np.arange(depth_min, depth_interval * self.ndepths + depth_min, depth_interval,
                                         dtype=np.float32)
                mask = self.read_mask_hr(mask_filename)
                depth = self.read_depth_hr(depth_filename)
